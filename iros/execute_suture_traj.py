...
for s in range(SEGNUM):
    ...
    for (i,mini_segment) in enumerate(mini_segments[s]):
        ...
        best_left_path = plan_follow_traj(robot, "leftarm", left_ee_link, left_hmats, ds_traj[:,:7])
        best_right_path = plan_follow_traj(robot, "rightarm", right_ee_link, right_hmats, ds_traj[:,7:], "leftarm", best_left_path)
